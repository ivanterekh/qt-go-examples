...
func main() {
	...
	for i := 0; i < dx; i++ {
		for j := 0; j < dy; j++ {
			img.SetPixelColor2(i, j, gui.NewQColor3(i*2%255, j*8%255, i*2%255, 255))
	...
	//View.ConnectPaintEvent(func(event *gui.QPaintEvent) {
	//	painter := gui.NewQPainterFromPointer(window)
	//	painter.DrawPixmap9(0,0, Item.Pixmap())
	//})
...
func drawpixel(x, y int) {
	...
	if mp {
		...
		img.Fill2( gui.NewQColor3(y%255, x%255, 255, 255))
		img.SetPixelColor2(x, y, gui.NewQColor3(255, 0, 255, 255))
